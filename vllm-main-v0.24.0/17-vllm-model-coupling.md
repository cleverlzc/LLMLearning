# vLLM 是如何Serving Llama/Qwen/DeepSeek 这些大模型的？一次推理请求中的全部交互与耦合点

> **系列**: vLLM 技术博客系列 | **类型**: 模型适配深潜篇
>
> vLLM 是推理框架，Llama/Qwen/DeepSeek 是具体的LLM模型——它们之间到底怎么"握手"？谁定义接口？谁适配谁？一次推理请求中，框架和模型在哪些点发生交互？本文逐个拆解每一个耦合点，让你彻底理解 vLLM 的"模型适配层"。

一个LLM模型（Llama/Qwen/DeepSeek等）通过vLLM系统提供高性能高并发推理服务，vLLM 到底是如何 Serving 这些LLM大模型的？如果你正要搭建一个高性能推理系统提供给公司或者部门使用，或者业界发布一个新的模型需要Day0适配上线，要做哪几步？本文将一探究竟。

当前 vLLM 已支持业界 200+ LLM大语言模型，文末附录支持列表。本文行文内容主要以 Llama、Qwen、DeepSeek举例。
